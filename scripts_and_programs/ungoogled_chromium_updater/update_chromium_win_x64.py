...
try:
    ...
    download_link = "https://github.com/ungoogled-software/ungoogled-chromium-windows/releases/download/" + latest_version_tag + "/ungoogled-chromium_" + latest_version_tag + "_installer_x64.exe"
    ...
    with open(latest_version_tag + "_installer_x64.exe", "wb") as installer_file:
        for chunk in installer_download.iter_content(chunk_size=1024): 
            if chunk:
                installer_file.write(chunk)

    print("Running installer")
    subprocess.run(latest_version_tag + "_installer_x64.exe", shell=True)

    print("Deleting installer")
    os.remove(latest_version_tag + "_installer_x64.exe")
